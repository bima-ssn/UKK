...
@section('content')
<div>
    <h2 class="text-2xl font-bold mb-6">Tambah Alat</h2>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('admin.alat.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nama_alat" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('nama_alat') border-red-500 @enderror" value="{{ old('nama_alat') }}" required>
                @error('nama_alat')
                    <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="stok" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('stok') border-red-500 @enderror" value="{{ old('stok', 0) }}" min="0" required>
                @error('stok')
                    <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                @enderror
            </div>
